...
def run_local_worker(
    ...
    # Artifact Finalization: Copy artifacts from artifact_dir to original repo before cleanup
    typer.echo("📦 Finalizing artifacts...")
    try:
        import shutil
        from pathlib import Path
        
        # The artifact_dir is local and contains files generated by Brain
        artifacts_copied = []
        
        # 1. Copy final_report.md if it exists
        final_report_src = artifact_dir / "final_report.md"
        if final_report_src.exists():
            final_report_dest = repo_path / "final_report.md"
            shutil.copy2(final_report_src, final_report_dest)
            artifacts_copied.append("final_report.md")
            typer.echo(f"   ✅ Copied final_report.md to {final_report_dest}")
        
        # 2. Copy cumulative_diff.patch if it exists (Brain might generate it)
        # Note: Brain might not generate this, but worker_service might during cleanup
        cumulative_diff_src = artifact_dir / "cumulative_diff.patch"
        if cumulative_diff_src.exists():
            cumulative_diff_dest = repo_path / "cumulative_diff.patch"
            shutil.copy2(cumulative_diff_src, cumulative_diff_dest)
            artifacts_copied.append("cumulative_diff.patch")
            typer.echo(f"   ✅ Copied cumulative_diff.patch to {cumulative_diff_dest}")
        
        if not artifacts_copied:
            typer.echo(f"   ℹ️  No artifacts found in {artifact_dir}")
    except Exception as e:
