...
def run_local_worker(
    ...
    try:
        ...
        final_report_src = Path(artifact_dir) / "final_report.md"
        if final_report_src.exists():
            final_report_dest = Path(repo_path) / "final_report.md"
            shutil.copy2(str(final_report_src), str(final_report_dest))
        ...
        cumulative_diff_src = Path(artifact_dir) / "cumulative_diff.patch"
        if cumulative_diff_src.exists():
            cumulative_diff_dest = Path(repo_path) / "cumulative_diff.patch"
            shutil.copy2(str(cumulative_diff_src), str(cumulative_diff_dest))
